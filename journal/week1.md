...
In Terraform, **"meta arguments"** are special settings you can use when declaring a resource that affect how Terraform manages that resource. They're like instructions to Terraform on how to handle the resource's lifecycle, dependencies, and other behaviors, and they help customize how your infrastructure is built and maintained. Meta arguments include for_each, lifecycle and many more.

### for_each 

The main usecase of **for_each** is to iterate or enumerate over complex data structures. It helps you avoid duplication and simplifies the management of multiple similar resources using a single concise definition. This is mostly useful when you are creating multiples of a cloud resource and you want to reduce the amount of repetitive terraform code.

In our project, **for_each** argument uses the list of filenames generated by **fileset()** to iterate and create one instance of the resource for each file that matches the pattern. 
...
To learn more about it, explore [for_each](https://developer.hashicorp.com/terraform/language/meta-arguments/for_each) page.
